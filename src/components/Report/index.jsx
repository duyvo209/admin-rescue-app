...
export default function App() {
  ...
  const getListReport = () => {
    db.collection('feedback').orderBy('time', 'desc').get().then( snapshot => {
      const report = []
      snapshot.forEach(doc => {
        const data = doc.data()
        const uid = doc.id
  ...
  const deleteReport = async (uid) => {
    await db.collection('feedback').doc(uid).update({
        'report': firebase.firestore.FieldValue.delete()
    })
  }
              ...
              <div className="content__area">
                ...
                <div className="content">
                  ...
                  <div className="commentUser">
                    ...
                    <div>
                        <p>{item.user_info.name}: {item.comment}</p>
                    </div>
